...
//   Phase 8.0 暗礁 27 证明: 配平点死区不补偿会导致姿态完全发散
//
// 补偿方法:
...
//
// 暗礁清单:
//   暗礁 29: Bouc-Wen gamma 符号修正 (+0.5, 标准公式 z 有界)
//   暗礁 37: 速率限制在死区补偿之前 (否则大指令被限到 < 死区被吞掉)
//   暗礁 38: TVC 延迟缓冲区未填满时返回 0 (纯延迟的物理特性)
//
...
//
// Bouc-Wen 模型 (Phase 9.0 修正, 暗礁 29):
...
//   - 限幅 (±10°)
//
// 暗礁 38: TVC 延迟缓冲区未填满时返回 0, 模拟纯延迟的物理特性.
...
public:
    ...
    //
    // 暗礁 38: 启动阶段 (t < delay) 返回 0, 模拟纯延迟的物理特性.
    // =========================================================================
    void update(const float gimbal_cmds[2], float dt,
        ...
        // 暗礁 38: 启动阶段 t < delay 时返回 0 (纯延迟的物理特性)
        if (t < delay) {
            return;
        }
